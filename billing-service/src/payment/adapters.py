"""Adapter fuer Stripe und PayPal."""
...
class PaymentAdapter:
    """Basisklasse fuer Zahlungsanbieter."""

    provider_name: str
    # Registrierung nach Anbietername.
    ...
    def get_status(self, transaction_id: str) -> PaymentResult:
        """Liest den Zahlungsstatus."""
        raise NotImplementedError


class StripeAdapter(PaymentAdapter):
    """Stripe-Adapter mit Sandbox- und Stub-Modus."""
    ...
    def charge(
        ...
        """Startet eine Stripe-Zahlung oder deren lokale Simulation."""
    ...
    def refund(self, transaction_id: str, amount: Decimal) -> PaymentResult:
        """Erstattet PaymentIntents per API und Stub-Zahlungen lokal."""
    ...
    def get_status(self, transaction_id: str) -> PaymentResult:
        """Liest den Status einer Stripe-Checkout-Session."""
        if settings.stripe_secret_key and not transaction_id.startswith("stripe-"):
            # Technische Fehler werden von der PaymentFacade wiederholt.
            session = self.retrieve_session(transaction_id)
            if session.get("paymentStatus") == "paid":
                # Refunds benoetigen die PaymentIntent-ID statt der Session-ID.
                return PaymentResult(
    ...
    def create_session(
        ...
        """Legt eine Stripe-Checkout-Session an."""
        data = {
            "mode": "payment",
        ...
        # Ohne Artikelliste wird der Gesamtbetrag als Sammelposten uebergeben.
        line_items = items or [
            {
    ...
    def retrieve_session(self, session_id: str) -> dict:
        """Liest Status, PaymentIntent und Kundendaten einer Checkout-Session."""
    ...
    def _request(self, url: str, data: dict, method: str = "POST") -> dict:
        """Fuehrt einen authentifizierten Request gegen die Stripe-API aus."""
...
class PayPalAdapter(PaymentAdapter):
    """Asynchroner PayPal-Adapter mit Sandbox- und Stub-Modus."""
    ...
    def charge(
        ...
        """Startet eine ausstehende PayPal-Zahlung."""
        payment_metadata = payment_metadata or {}
        simulated = _simulated_result(self.provider_name, order_id, payment_metadata)
        if simulated:
            return simulated
        ...
        # Im Stub-Modus bestimmt payment_metadata das Webhook-Ergebnis.
    ...
    def refund(self, transaction_id: str, amount: Decimal) -> PaymentResult:
        """Markiert eine PayPal-Zahlung lokal als erstattet."""
        return PaymentResult(
            transaction_id=transaction_id,
            provider=self.provider_name,
            status=PaymentStatus.REFUNDED,
    ...
    def get_status(self, transaction_id: str) -> PaymentResult:
        """Fuehrt den PayPal-Capture aus und liefert dessen Status."""
    ...
    def create_order(
        ...
        """Legt eine PayPal-Order mit Capture-Absicht an."""
    ...
    def capture_order(self, paypal_order_id: str) -> dict:
        """Schliesst eine freigegebene PayPal-Order ab."""
        body = self._paypal_json_request(f"/v2/checkout/orders/{paypal_order_id}/capture", {})
        capture_id = None
        shipping_address = None
        # PayPal kann mehrere purchase_units liefern.
    ...
    def _schedule_webhook(self, payload: dict) -> None:
        """Plant den verzoegerten Webhook des Stub-Modus."""
    ...
    def _send_webhook(self, payload: dict) -> None:
        """Sendet den simulierten Webhook an den Billing-Service."""
        request = Request(
    ...
    def _paypal_json_request(self, path: str, payload: dict) -> dict:
        """Fuehrt einen authentifizierten JSON-Request gegen PayPal aus."""
    ...
    def _access_token(self) -> str:
        """Holt ein OAuth2-Token per Client-Credentials-Flow."""
        request = Request(
...
def _with_real_content(normalized: dict | None, *keys: str) -> dict | None:
    """Verwirft normalisierte Datensaetze, die nur Platzhalter enthalten."""
    if not normalized:
        return None
...
def _minor_units(amount: Decimal) -> int:
    """Wandelt einen Betrag in die kleinste Waehrungseinheit um."""
    return int((amount * Decimal("100")).quantize(Decimal("1")))


def _simulated_result(provider: str, order_id: str, payment_metadata: dict) -> PaymentResult | None:
    """Liefert das konfigurierte Fehlerergebnis fuer Tests und Demos."""
...
def _normalize_paypal_payer(payer: dict) -> dict:
    """Normalisiert PayPal-Kundendaten."""
    name = payer.get("name", {})
    return {
...
def _normalize_paypal_shipping(shipping: dict) -> dict:
    """Normalisiert eine PayPal-Lieferadresse."""
...
def _split_street_and_house_number(address_line: str) -> tuple[str, str]:
    """Trennt die letzte Hausnummer von einer Adresszeile."""
...
def _normalize_stripe_customer(customer_details: dict) -> dict:
    """Normalisiert Stripe-Kundendaten."""
...
def _normalize_stripe_shipping(shipping_details: dict) -> dict:
    """Normalisiert eine Stripe-Lieferadresse."""
...
def _stripe_shipping_details(session: dict) -> dict:
    """Liest die Lieferadresse aus den unterstuetzten Stripe-Antwortformaten."""
...
def _split_full_name(name: str) -> tuple[str, str]:
    """Trennt einen Namen am ersten Leerzeichen."""

...
def consume_messages(
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            # z.B. genau waehrend eines publish_message()-Aufrufs innerhalb von
            # handle_message() aus, kann der anschliessende
            # channel.basic_nack()-Aufruf unten (im inneren except Exception)
            # selbst noch eine weitere, eventuell nicht gelistete pika-
            # Exception werfen (z.B. ChannelWrongStateError). Wird die nicht
            # hier aufgefangen, stirbt der Consumer-Thread lautlos und
            # verbindet sich nie wieder neu.
            if stop_event.is_set():
                return

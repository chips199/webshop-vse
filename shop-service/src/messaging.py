    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            for routing_key in routing_keys:
                channel.queue_bind(queue=SHOP_QUEUE_NAME, exchange=EXCHANGE_NAME, routing_key=routing_key)
            logger.info("RabbitMQ shop consumer connected")
            ...
            if not stop_event.is_set():
                logger.exception("RabbitMQ shop consumer disconnected; retrying")
                time.sleep(2)
        finally:
            if channel and channel.is_open:
                try:
                    ...
                    logger.exception("Failed to cancel shop consumer channel")

"""Append-only-Datenbankzugriff des Audit-Service."""
...
# previous_event_id bleibt ohne Fremdschluessel, da Ereignisse ungeordnet eintreffen koennen.
...
def init_database() -> None:
    """Initialisiert Erweiterung, Tabelle und Index."""
...
def get_snapshots_by_correlation_id(correlation_id: str) -> Iterable[dict[str, Any]]:
    """Liest Snapshots chronologisch nach Ereignis- und Speicherzeitpunkt."""
    query = """
    SELECT
        id,
...
def insert_snapshot_from_message(message: dict[str, Any]) -> None:
    """Speichert eine Nachricht idempotent als Audit-Snapshot."""
    query = """
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                ...
                (
                    ...
                    # Beispiel: billing.payment.failed -> BILLING_PAYMENT_FAILED.
...
def _status_code_for(message_type: str) -> str:
    """Leitet den Audit-Status aus dem Nachrichtentyp ab."""
